   ...
   try {
      ...
      for (let index = 0; index < listcar.length; index++) {
         ...
         var j = schedule.scheduleJob(
            { start: starttime, end: endtime, rule: "* * */24 * * *" },
            async function () {
                  ...
                  if (d.email) {
                     const notification = {
                        ...
                        subject:
                           "Xe " +
                           listcar[index]["plate"] +
                           " Sắp hết hạn vui lòng đăng ký lại",
                        html:
                           `<p>Bạn cần đăng ký lại bảo hiểm xe trước ngày </p>` +
                           listcar[index]["insurance"]["expired"],
                     };
                     const sendMailSuccess = await sendAutoMail(notification);
                     console.log(notification);
                     console.log("gui thanh cong");
                     if (!sendMailSuccess)
                        sendError(res, "Send Mail to carfleet Faild");
                  } else {
                     const notification = {
                        to: d.phone,
                     ...
                     if (!sendSMSSucsses)
                        sendError(res, "Send SMS to Carfleer Faild");
                  }
               ...
               const title =
                  ...
                  " Sắp hết hạn vui lòng đăng ký lại";
               const message =
                  `Bạn cần đăng ký lại bảo hiểm xe trước ngày` +
                  listcar[index]["insurance"]["expired"];
                  ...
                  if (drvierEmail) {
                     const notification = {
                        ...
                        html:
                           ...
                           listBill[i].road +
                           "<br>" +
                           "<p> Product shipments: </p>" +
                           listBill[i].product_shipments +
                           "<br>",
                     };
                     const sendMailSuccess = await sendAutoMail(notification);
                     console.log(notification);
                     ...
                     if (!sendMailSuccess)
                        sendError(res, "Send Mail to driver failed");
                  } else {
                     const notification = {
                        to: drvierPhone,
                     ...
                     const sendSMSSucsses = await sendAutoSMS(notification);
                     if (!sendSMSSucsses)
                        sendError(res, "Send SMS to driver failed");
                  }
